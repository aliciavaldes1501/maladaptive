...
## Option 1 (probably not used): use plots to calculate correlations between soil and air logger temperatures
...
(logger_data%>%
  ...
  xlab("Plot mean soil temperature for April-May-June")+
...
(logger_data%>%
  ...
  ggplot(.,aes(x=meansoiltemp,y=corr,color=measure))+geom_point()+
  geom_smooth(method="lm")+
  xlab("Plot mean soil temperature for May")+
  ylab("Correlation coef. air-soil temperature\nover May\n(based on plot daily mean/min/max temperatures)")
```
...
Linear models testing the effect of soil temperature on correlations between soil and air temperature, removing pairs with negative correlations (5-6 pairs):
...
(logger_data_pairs%>%
  ...
  geom_smooth(method="lm")+
  xlab("Mean soil temperature for April-May-June")+
  ylab("Correlation coef. air-soil temperature\nover April-May-June\n(based on daily mean/min/max DAY temperatures)")+
  geom_text_repel(data=. %>% filter(corr<0),aes(label=pair))
```
...
(logger_data_pairs%>%
  ...
  geom_smooth(method="lm")+
  xlab("Mean soil temperature for April-May-June")+
  ylab("Correlation coef. air-soil temperature\nover April-May-June\n(based on daily mean/min/max POSITIVE temperatures)")+
  geom_text_repel(data=. %>% filter(corr<0),aes(label=pair))
```
...
Linear models testing the effect of soil temperature on correlations between soil and air temperature, removing pairs with negative correlations:
...
#### Correlations soil-air temperature for June 1-5 

##### Based on 24-h values

For each date and logger pair, calculate mean, max and min of air and soil temperature (from, respectively, the above and belowground logger). Then, calculate the correlation coefficient for air and soil temperatures over the month of May. Finally, regress these correlation coefficients on mean soil temperature (from the belowground logger) **for the same period (June)**.
...
(logger_data_pairs%>%
  ...
  ggplot(.,aes(x=meansoiltemp,y=corr,color=measure))+geom_point()+
  geom_smooth(method="lm")+
  xlab("Mean soil temperature for June 1-5")+
  ylab("Correlation coef. air-soil temperature\nover June 1-5\n(based on daily mean/min/max temperatures)")#+
  #geom_text_repel(data=. %>% filter(corr<0),aes(label=pair))
```
...
# Q3 Option 2 with logger pairs with no problems
