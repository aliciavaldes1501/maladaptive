...
id_original included as a random factor to account for repeated measures (69 individuals were common to both study years).

```{r echo=TRUE}
data_plants$year_fct<-as.factor(data_plants$year)
FFD_1<-glmmTMB(ffd~(temp+I(temp^2))*year_fct+(1|id_original),data_plants)
summary(FFD_1)
```
...
```{r echo=TRUE}
FFD_1<-glmmTMB(ffd~temp*year_fct+(1|id_original),data_plants)
summary(FFD_1)
```
...
```{r echo=TRUE}
fitness_1<-glmmTMB(n_seed_round~(temp+I(temp^2))*year_fct+log(nfl)+
                      (1|id_original),data_plants,family="nbinom2")
summary(fitness_1)
```
...
```{r echo=TRUE}
data_plants$n_fl_log<-log(data_plants$nfl)
fitness_1<-glmmTMB(n_seed_round~temp*year_fct+n_fl_log+
                      (1|id_original),data_plants,family="nbinom2")
summary(fitness_1)
```

Interaction year × soil temperature not significant. No need to fit separate models for each year.

Predictions of fitness for minimum and maximum temperatures:

```{r}
range(data_plants$temp)
ggpredict(fitness_1,terms=c("temp[3.5,45.5]"))
```
...
Model prediction fitness : based on global model

```{r}
predict_fitness<-ggpredict(fitness_1,terms=c("temp [all]"))
```
...
fig2<-
  grid.arrange(
    ...
    ggplot()+my_theme()+
      ...
      geom_ribbon(data=predict_fitness,
                  aes(x=x,y=predicted,ymin=conf.low,ymax=conf.high),
                  alpha=0.5)+
      geom_line(data=predict_fitness,
...
```{r echo=TRUE}
selection_1<-glmmTMB(nseed_rel~ffd_std*(temp+I(temp^2))*year_fct+nfl_std+
                       (1|id_original),data_plants)
summary(selection_1)
```
...
```{r echo=TRUE}
selection_1<-glmmTMB(nseed_rel~ffd_std*temp*year_fct+nfl_std+
                      (1|id_original),data_plants)
summary(selection_1)
```
...
Refit model with lmer.

```{r}
selection_1_lmer<-lmer(nseed_rel~ffd_std*temp*year_fct+nfl_std+
                      (1|id_original),data.frame(data_plants))
summary(selection_1_lmer)
```


```{r eval=FALSE, include=FALSE}
b_par<-bootMer(x=selection_1_lmer,FUN=fixef,nsim=10000)
save(b_par,file="output/b_par.RData")
```
