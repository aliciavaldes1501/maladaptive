...
coords_pls_2017<-read.dbf("gis/coords_pls_2017.dbf")
coords_pls_2018<-read.dbf("gis/coords_pls_2018.dbf")
coords_pls_2017$year<-2017
coords_pls_2018$year<-2018
coords_pls_2017$id_original<-coords_pls_2017$pl_id
coords_pls_2017$pl_id<-NULL
coords_pls_2018$id_original<-coords_pls_2018$pl_id
coords_pls_2018$pl_id<-NULL
coords_pls_2017$Id<-NULL
coords_pls_2018$Id<-NULL
data_plants<-data_plants%>%left_join(rbind(coords_pls_2017,coords_pls_2018))
...
```{r}
euclidDist <- sp::spDists(data_plants[c(1:4),],longlat = FALSE)
```
...
### Appendix S1: Correlations soil-air temperature vs soil temperature (April-June)

```{r fig.height=3, fig.width=4, message=FALSE, warning=FALSE}
AppS1<-(logger_data_pairs%>%
          mutate(month = month(datetime),date=date(datetime))%>%
          # new variables "month" and "date"
  ...
  geom_text_repel(data=. %>% filter(corr<0),aes(label=pair))
AppS1
ggsave(filename="output/figures/AppS1.tiff",plot=AppS1,
       width=12,height=10,units="cm",dpi=300)
```
...
Quadratic terms of ffd not significant. Refit models without quadratic terms of ffd.
...
fig3<-
  grid.arrange(
    # ffd
    ggplot(data.frame(data_plants),aes(x=temp,y=ffd))+
  ...
  scale_fill_manual(values=cbPalette)+scale_color_manual(values=cbPalette),
    # fitness
    ggplot(data.frame(data_plants),aes(x=temp,y=nseed))+
...
# Figure 4: Effects of temperature on selection

```{r}
quantile(subset(data_plants,year==2017)$temp,probs=c(0.05,0.95))
quantile(subset(data_plants,year==2018)$temp,probs=c(0.05,0.95))
pred_fitness_17<-ggpredict(selection_1,
                           terms = c("ffd_std[all]","temp[6.32:26.90]",
                                     "year_fct[2017]"))
pred_fitness_18<-ggpredict(selection_1,
                           terms = c("ffd_std[all]","temp[5.060:25.825]",
                                     "year_fct[2018]"))
```

```{r fig.height=3, fig.width=6}
legend <- as_ggplot(get_legend(ggplot(pred_fitness_17,
                          aes(x,predicted,colour=group,fill=group))+
  geom_line(aes(color=as.numeric(as.character(group))),size=0.5)+my_theme()+
  scale_color_viridis()+
  theme(legend.position=c(0.5,0.75))+labs(colour="Soil\ntemperature\n(ºC)")+
  xlab("Standardized FFD")+ylab(NULL)+ggtitle("A) 2017")))
fig4old<-grid.arrange(
  ggplot(pred_fitness_17,aes(x,predicted,colour=group,fill=group))+
    geom_line(aes(color=as.numeric(as.character(group))),size=0.5)+my_theme()+
    scale_color_viridis()+labs(colour="Soil temperature (ºC)")+
    xlab("Standardized FFD")+ylab(NULL)+ggtitle("A) 2017")+
    scale_y_continuous(limits=c(-0.4,2.8),breaks=c(0,0.5,1,1.5,2,2.5)),
  ggplot(pred_fitness_18,aes(x,predicted,colour=group,fill=group))+
    geom_line(aes(color=as.numeric(as.character(group))),size=0.5)+my_theme()+
    scale_color_viridis()+labs(colour="Soil temperature (ºC)")+
    xlab("Standardized FFD")+ylab(NULL)+ggtitle("B) 2018")+
    scale_y_continuous(limits=c(-0.4,2.8),breaks=c(0,0.5,1,1.5,2,2.5)),
  legend,
  ncol=3,left=textGrob("Relative fitness",rot=90,just=0.7,
                       gp=gpar(fontsize=16,fontfamily="serif")),
  widths=c(1,1,0.3))
ggsave(filename="output/figures/fig4old.tiff",plot=fig4old,
       width=25,height=12,units="cm",dpi=300)
```

```{r}
fig4<-cowplot::plot_grid(ggplot(data.frame(data_plants),
...
pred_fitness_17_cats<-rbind(
  (data.frame(ggpredict(selection_1,
                           terms = c("ffd_std[all]","temp[6.714286]",
                                     "year_fct[2017]")))%>%
     mutate(temp_cat=1)%>%
     dplyr::rename(FFD_std=x, fitness=predicted,temp=group)),
  (data.frame(ggpredict(selection_1,
                        terms = c("ffd_std[all]","temp[8.748333]",
                                     "year_fct[2017]")))%>%
     mutate(temp_cat=2)%>%
     dplyr::rename(FFD_std=x, fitness=predicted,temp=group)),
  (data.frame(ggpredict(selection_1,
                        terms = c("ffd_std[all]","temp[16.2129]",
                                     "year_fct[2017]")))%>%
     mutate(temp_cat=3)%>%
     dplyr::rename(FFD_std=x, fitness=predicted,temp=group)),
  (data.frame(ggpredict(selection_1,
                        terms = c("ffd_std[all]","temp[25.10667]",
                                     "year_fct[2017]")))%>%
     mutate(temp_cat=4)%>%
     dplyr::rename(FFD_std=x, fitness=predicted,temp=group)))
...
```{r}
plot(ggpredict(selection_1,
               terms=c("ffd_std[all]","temp[6.5:6.7 by=0.1]","year_fct[2018]")),
     ci=F)
```

In 2018, the model predicted that selection favoured earlier flowering at soil temperatures up to 6.5 ºC, while later flowering was favoured at higher soil temperatures.
...
I think it makes no sense to test for spatial autocorrelation in the residuals of models with year as a factor, because we are mixing plants for different years.

By now, I will run separate models for each year and test for residual spatial autocorrelation in those.
...
spline.correlog_FFD_2017 <- spline.correlog(x=subset(data_plants,year==2017)$x,
                            y=subset(data_plants,year==2017)$y,
                            res_FFD_2017,resamp=100,quiet=F,type="boot") 
spline.correlog_FFD_2018 <- spline.correlog(x=subset(data_plants,year==2018)$x,
                            y=subset(data_plants,year==2018)$y,
                            res_FFD_2018,resamp=100,quiet=F,type="boot") 
```

```{r}
plot(correlog_FFD_2017)
plot(correlog_FFD_2018)
plot(spline.correlog_FFD_2017)
plot(spline.correlog_FFD_2018)
```
...
spline.correlog_FFD_2017_ME <- spline.correlog(x=subset(data_plants,year==2017)$x,
                            y=subset(data_plants,year==2017)$y,
                            res_FFD_2017_ME,resamp=100,quiet=F,type="boot") 
spline.correlog_FFD_2018_ME <- spline.correlog(x=subset(data_plants,year==2018)$x,
                            y=subset(data_plants,year==2018)$y,
                            res_FFD_2018_ME,resamp=100,quiet=F,type="boot") 
```

```{r}
plot(correlog_FFD_2017_ME)
plot(correlog_FFD_2018_ME)
plot(spline.correlog_FFD_2017_ME)
plot(spline.correlog_FFD_2018_ME)
```
...
ggsave(filename="output/figures/AppS5_FigS1.tiff",
       plot=AppS5_FigS1,device="tiff",width=28,height=12,units="cm",dpi=300,
       compression="lzw")
```
...
spline.correlog_fitness_2017 <- spline.correlog(x=subset(data_plants,year==2017)$x,
                            y=subset(data_plants,year==2017)$y,
                            res_fitness_2017,resamp=100,quiet=F,type="boot") 
spline.correlog_fitness_2018 <- spline.correlog(x=subset(data_plants,year==2018)$x,
                            y=subset(data_plants,year==2018)$y,
                            res_fitness_2018,resamp=100,quiet=F,type="boot") 
```

```{r}
plot(correlog_fitness_2017)
plot(correlog_fitness_2018)
plot(spline.correlog_fitness_2017)
plot(spline.correlog_fitness_2018)
```
...
spline.correlog_fitness_2017_ME <- spline.correlog(x=subset(data_plants,year==2017)$x,
                            y=subset(data_plants,year==2017)$y,
                            res_fitness_2017_ME,resamp=100,quiet=F,type="boot") 
```

```{r}
plot(correlog_fitness_2017_ME)
plot(spline.correlog_fitness_2017_ME)
```
...
ggsave(filename="output/figures/AppS5_FigS2.tiff",
       plot=AppS5_FigS2,device="tiff",width=28,height=12,units="cm",dpi=300,
       compression="lzw")
```
...
spline.correlog_selection_2017 <- spline.correlog(x=subset(data_plants,year==2017)$x,
                            y=subset(data_plants,year==2017)$y,
                            res_selection_2017,resamp=100,quiet=F,type="boot") 
spline.correlog_selection_2018 <- spline.correlog(x=subset(data_plants,year==2018)$x,
                            y=subset(data_plants,year==2018)$y,
                            res_selection_2018,resamp=100,quiet=F,type="boot") 
```

```{r}
plot(correlog_selection_2017)
plot(correlog_selection_2018)
plot(spline.correlog_selection_2017)
plot(spline.correlog_selection_2018)
```
...
spline.correlog_selection_2017_ME <- spline.correlog(x=subset(data_plants,year==2017)$x,
                            y=subset(data_plants,year==2017)$y,
                            res_selection_2017_ME,resamp=100,quiet=F,type="boot") 
```

```{r}
plot(correlog_selection_2017_ME)
plot(spline.correlog_selection_2017_ME)
```
...
ggsave(filename="output/figures/AppS5_FigS3.tiff",
       plot=AppS5_FigS3,device="tiff",width=28,height=12,units="cm",dpi=300,
       compression="lzw")
```
...
spline.correlog_selectionabs_2017 <- spline.correlog(x=subset(data_plants,
                                                              year==2017)$x,
                            y=subset(data_plants,year==2017)$y,
                            res_selectionabs_2017,resamp=100,quiet=F,type="boot") 
spline.correlog_selectionabs_2018 <- spline.correlog(x=subset(data_plants,
                                                              year==2018)$x,
                            y=subset(data_plants,year==2018)$y,
                            res_selectionabs_2018,resamp=100,quiet=F,type="boot") 
```
...
plot(spline.correlog_selectionabs_2017)
plot(spline.correlog_selectionabs_2018)
```
...
spline.correlog_selectionabs_2017_ME <- spline.correlog(x=subset(data_plants,year==2017)$x,
                            y=subset(data_plants,year==2017)$y,
                            res_selectionabs_2017_ME,resamp=100,quiet=F,type="boot") 
```

```{r}
plot(correlog_selectionabs_2017_ME)
plot(spline.correlog_selectionabs_2017_ME)
```
...
ggsave(filename="output/figures/AppS5_FigS4.tiff",
       plot=AppS5_FigS4,device="tiff",width=28,height=12,units="cm",dpi=300,
       compression="lzw")
```
