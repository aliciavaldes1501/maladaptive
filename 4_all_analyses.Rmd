...
# 2. Effect of temperature on plant size

For 2017, get data on rosette diameter, maximum and median height from previous ms.

```{r message=FALSE, warning=FALSE}
size_2017<-read.csv("C:/Users/User/Dropbox/SU/Projects/cerastium/data/clean/cerastium_2017_FFD_temp_fit.txt",sep="\t")%>%
  mutate(totalh=ifelse(nstems==0,NA,nstems*medianh))%>%
  dplyr::select(id,temp1,maxh,medianh,area,totalh)%>%
  rename(id_original=id)%>%
  mutate(year=2017)%>%
  mutate(year=as.factor(year))
size_2018<-read_csv("data/edited/size_2018.csv")%>%
  mutate(area=pi*d1*d2,totalh=ifelse(is.na(nstems),NA,nstems*medianh),
         year=2018)%>%
  mutate(year=as.factor(year))%>%
  dplyr::select(id_original,temp1,maxh,medianh,area,totalh,year)
mydata_size<-rbind(size_2017,size_2018)
```

Distributions 

```{r message=FALSE, warning=FALSE}
pivot_longer(mydata_size,cols=3:6) %>% 
  ggplot(aes(value)) +
    facet_wrap(~ name, scales = "free") +
    geom_histogram(fill="white", color="black")
```
...
Models of area, maximum height, median height and total height vs soil temperature

# 2018

```{r}
area1_2018<-lm(area~temp1,subset(mydata_size,year==2018))
maxh1_2018<-lm(maxh~temp1,subset(mydata_size,year==2018))
medianh1_2018<-lm(medianh~temp1,subset(mydata_size,year==2018))
totalh1_2018<-lm(totalh~temp1,subset(mydata_size,year==2018))
```
...
Significant negative effect of temperature on median height.

## Model diagnostics

```{r}
simulationOutput_medianh1_2018<-simulateResiduals(fittedModel=medianh1_2018,
                                                  n=5000)
```
...
```{r fig.height=4, fig.width=7}
plot(simulationOutput_medianh1_2018)
```

Residuals against temperature

```{r}
plotResiduals(simulationOutput_medianh1_2018,
     form = subset(mydata_size,year==2018&!is.na(medianh))$temp1)
```

Predictions of fitness for minimum and maximum temperatures:

```{r}
ggpredict(medianh1_2018,terms="temp1[minmax]") 
```
# 2017

```{r}
area1_2017<-lm(area~temp1,subset(mydata_size,year==2017))
maxh1_2017<-lm(maxh~temp1,subset(mydata_size,year==2017))
medianh1_2017<-lm(medianh~temp1,subset(mydata_size,year==2017))
totalh1_2017<-lm(totalh~temp1,subset(mydata_size,year==2017))
```
...
Marginally significant (p=0.06) negative effect of temperature on median height. Significant positive effect of temperature on total height, but I am not sure this variable is OK to use.

## Model diagnostics

```{r}
simulationOutput_medianh1_2017<-simulateResiduals(fittedModel=medianh1_2017,
                                                  n=5000)
```
...
```{r fig.height=4, fig.width=7}
plot(simulationOutput_medianh1_2017)
```

Residuals against temperature

```{r}
plotResiduals(simulationOutput_medianh1_2017,
     form = subset(mydata_size,year==2017&!is.na(medianh))$temp1)
```

# Both years

```{r}
area1<-lm(area~temp1,mydata_size)
maxh1<-lm(maxh~temp1,mydata_size)
medianh1<-lm(medianh~temp1,mydata_size)
totalh1<-lm(totalh~temp1,mydata_size)
```
...
```{r}
simulationOutput_medianh1<-simulateResiduals(fittedModel=medianh1,
                                                  n=5000)
```
...
```{r fig.height=4, fig.width=7}
plot(simulationOutput_medianh1)
```

Residuals against temperature

```{r}
plotResiduals(simulationOutput_medianh1,
     form = subset(mydata_size,!is.na(medianh))$temp1)
```
...
# Figure 2: Effects of temperature on ffd, size and fitness in 2018
...
Model predictions size : based on models medianh1_2017 and medianh1_2018.

```{r}
predict_size_2017<-ggpredict(medianh1_2017,terms = "temp1 [all]")
predict_size_2018<-ggpredict(medianh1_2018,terms = "temp1 [all]")
```
...
fig2<-
  grid.arrange(
    ...
    ggplot(subset(mydata,year==2018),aes(x=temp,y=ffd))+
      ...
      ggtitle("A)")+theme(plot.title=element_text(hjust=-0.22,vjust=-3))+
      theme(plot.margin = unit(c(-0.6,0.3,0,0.3), "cm")),
    #size
    ggplot(subset(mydata_size,year==2018),aes(x=temp1,y=medianh))+
      xlab("Soil temperature (ºC)")+ylab("Size (cm)")+
      my_theme()+
      geom_ribbon(data=predict_size_2018,
                  aes(x=x,y=predicted,ymin=conf.low,ymax=conf.high),
                  fill="grey",alpha=0.7)+
      geom_line(data=predict_size_2018,
                aes(x=x,y=predicted),size=1,color="black")+
      geom_point(size=2,alpha=0.2)+
      ggtitle("B)")+theme(plot.title=element_text(hjust=-0.22,vjust=-3))+
      theme(plot.margin = unit(c(-0.6,0.3,0,0.3), "cm")),
    # fitness
    ggplot(subset(mydata,year==2018),aes(x=temp,y=nseed))+
      ...
      geom_line(data=predict_fitness_2018,
                aes(x=x,y=predicted),size=1,color="black")+
      geom_point(size=2,alpha=0.2)+
      ggtitle("C)")+theme(plot.title=element_text(hjust=-0.22,vjust=-3))+
      theme(plot.margin = unit(c(-0.6,0.3,0,0.3), "cm")),
    ncol=3)
ggsave(filename="output/figures/fig2.tiff",plot=fig2,
       width=30,height=9,units="cm",dpi=300)
```
...
figS1<-
  grid.arrange(
    ...
    ggplot(subset(mydata,year==2017),aes(x=temp,y=ffd))+
      ...
      theme(plot.margin = unit(c(-0.6,0.3,0,0.3), "cm")),
    #size
    ggplot(subset(mydata_size,year==2017),aes(x=temp1,y=medianh))+
      xlab("Soil temperature (ºC)")+ylab("Size (cm)")+
      my_theme()+
      geom_ribbon(data=predict_size_2017,
                  aes(x=x,y=predicted,ymin=conf.low,ymax=conf.high),
                  fill="grey",alpha=0.7)+
      geom_line(data=predict_size_2017,
                aes(x=x,y=predicted),size=1,color="black")+
      geom_point(size=2,alpha=0.2)+
      ggtitle("B)")+theme(plot.title=element_text(hjust=-0.22,vjust=-3))+
      theme(plot.margin = unit(c(-0.6,0.3,0,0.3), "cm")),
...
# 4. How useful soil temperature is as a cue for air temperature
...
Use only logger pairs with no problems (53 pairs out of 73): pairs 1-4,9,11,13-16,21-62,70.
...
logger_data_pairs<-logger_data_pairs%>%
  mutate(pair_problem=ifelse(pair%in%c(1:4,9,11,13:16,21:62,70),0,1))%>%
...
68 loggers (34 pairs), of which 32 loggers in Cerastium plants, and 36 loggers in other plants.
...
## Figure 3: Correlations soil-air temperature vs soil temperature

```{r fig.height=6, fig.width=12, message=FALSE, warning=FALSE}
plot1<-(logger_data_pairs%>%
          mutate(month = month(datetime))%>% # new variable "month"
          filter(month==5)%>% # keep data from may
          group_by(date,pair,above_below)%>% 
          summarise(mean=mean(temp,na.rm=T),max=max(temp),min=min(temp),
  ...
  xlab("Mean soil temperature")+
  ylab("Correlation coefficient\nsoil-air temperature")+
  my_theme()+ggtitle("A) May")+
  theme(axis.title.y=element_blank())
plot2<-(logger_data_pairs%>%
          mutate(month = month(datetime))%>% # new variable "month"
          filter(month==4|month==5|month==6)%>% # keep data from april-june
          group_by(date,pair,above_below)%>% 
          summarise(mean=mean(temp,na.rm=T),max=max(temp),min=min(temp),
  ...
  xlab("Mean soil temperature")+
  ylab("Correlation coefficient\nsoil-air temperature")+
  my_theme()+ggtitle("B) April-June")+  
  theme(axis.title.y=element_blank(),axis.text.y=element_text(color="white"))
fig3<-grid.arrange(plot1,plot2,ncol=2,
             left=textGrob("Correlation coefficient\nsoil-air temperature",
                           rot=90,gp=gpar(fontsize=16,font=6)))
...
ggplot(maytemps,aes(x=temp_term,y=meanmay_logger))+
  ...
  ylab("Mean temperature of May\nfrom logger data")+my_theme()
summ(lm(meanmay_logger~temp_term,maytemps))
ggsave(filename="output/figures/appendix.tiff",width=10,height=8,
       units="cm",dpi=300)
```
...
# 5. Effect of temperature on selection on FFD
...
# 6. Effect of temperature on the relationship absolute fitness-FFD
